...
local config = {
	...
	damage_meter_UI = {
		...
		include_small_monsters = false,
		include_player_damage = true,
		include_bomb_damage = true,
		include_kunai_damage = true,
		include_installation_damage = false, -- hunting_installations like ballista, cannon, etc.
		include_otomo_damage = false,
		include_monster_damage = false, -- note that installations during narwa fight are counted as monster damage
		...
		include_hunter_rank = true,
		include_player_id = false,
...
draw_player_label = function(label, position, hunter_rank, player_id, player_name)
	if label == nil then
		return;
	end
	...
	local text;
	--loop through (maybe a function table/switch would be better, but inexperienced in lua lol)
	--create and use a temp text_format if all player labels in damage ui are not set to true
	--include all labels
	if (config.damage_meter_UI.include_hunter_rank and config.damage_meter_UI.include_player_id and config.damage_meter_UI.include_player_name) then
		text = string.format(label.text, hunter_rank, player_id, player_name);
	--exclude hunter rank if false
	elseif (not config.damage_meter_UI.include_hunter_rank and config.damage_meter_UI.include_player_id and config.damage_meter_UI.include_player_name) then
		local text_format = "%d %s"
		text = string.format(text_format, player_id, player_name);
	--exclude hunter rank and player id if false
	elseif (not config.damage_meter_UI.include_hunter_rank and not config.damage_meter_UI.include_player_id and config.damage_meter_UI.include_player_name) then
		text_format = "%s"
		text = string.format(text_format, player_name);
	--exclude player id if false
	elseif (config.damage_meter_UI.include_hunter_rank and not config.damage_meter_UI.include_player_id and config.damage_meter_UI.include_player_name) then
		local text_format = "[%d] %s"
		text = string.format(text_format, hunter_rank, player_name);
	else --what are you even doing... why would you want to just display hunter rank? :D
		return;
	end
...
damage_meter = function()
	...
	for _, player in ipairs(quest_players) do
		...
		draw_player_label(config.damage_meter_UI.player_name_label, position_on_screen, player.hunter_rank, player.id, player.name)
